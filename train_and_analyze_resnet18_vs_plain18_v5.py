#!/usr/bin/env python3
# train_and_analyze_resnet_variants_correct_init.py
#
# Purpose:
#   1. Train two networks (resnet14, plain14) with reduced channels [8, 16, 32, 64]
...
#   3. Save training curves for analysis.
...
# ───────────────────────── CONFIG ─────────────────────────
BASE = Path.cwd()
CHK_DIR = BASE / "checkpoints_correct_init"
CHK_DIR.mkdir(exist_ok=True)

CURVE_DIR = BASE / "viz_correct_init"
CURVE_DIR.mkdir(exist_ok=True)

PLOT_DIR = BASE / "viz_correct_init" / "plots" / "act_analysis_rvp_correct_init"
...
class ResNet(nn.Module):
    ...
    Modified for reduced channel configuration: [8, 16, 32, 64]
    """
    def __init__(self, block, layers: List[int], num_classes=1000):
        super().__init__()
        self.in_channels = 8  # Start with 8 channels instead of 64
        ...
        # Modified channel progression: [8, 16, 32, 64] instead of [64, 128, 256, 512]
        ...
        self.fc = nn.Linear(64 * block.expansion, num_classes)  # 64 instead of 512
...
# Only include the two models we want to train
MODELS = {
    "resnet14": resnet14(),
    "plain14":  plain14(),
}
...
DEPTH_CFG = {
    14: (100, 0.1, [50, 75]),  # epochs, initial LR, milestones
}
...
def parse_depth_from_tag(tag: str) -> int:
    """
    E.g. "resnet14" or "plain14" -> returns 14 as integer
    """
    m = re.search(r"(\d+)$", tag)
    if m:
...
def plot_training_curves(tag: str, train_losses: List[float], train_accs: List[float], 
    ...
    ax1.set_title(f'{tag.upper()} - Training & Validation Loss', fontsize=14, fontweight='bold')
    ax1.legend()
    ...
    ax2.set_title(f'{tag.upper()} - Training & Validation Accuracy', fontsize=14, fontweight='bold')
    ax2.legend()
    ax2.grid(True, alpha=0.3)
    
    plt.tight_layout()
    curve_path = CURVE_DIR / f"{DATASET}_{tag}_training_curves_correct_init.png"
    plt.savefig(curve_path, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"[INFO] Training curves saved to: {curve_path}")

# ───────── TRAIN LOOP ─────────
def train(tag: str, model: nn.Module, train_loader: DataLoader, val_loader: DataLoader):
    ck = CHK_DIR / f"{DATASET}_{tag}_correct_init.pth"
    if ck.exists():
    ...
    plot_training_curves(tag, train_losses, train_accs, val_losses, val_accs)
...
def _plot_box(activations: List[np.ndarray],
    ...
    plt.tight_layout()
    outpath = PLOT_DIR / f"{label}_box_correct_init.png"
    plt.savefig(outpath)
    plt.close()
...
def analyse(tag: str, model: nn.Module, results: Dict[str, Dict[str, List[float]]]):
    ...
    results[tag] = stats
    (PLOT_DIR / f"{DATASET}_{tag}_correct_init.json").write_text(json.dumps(stats))
    ...
    def _plot_line(vals, stds, title_, ylabel_, fname_):
        ...
        plt.grid(ls="--", lw=0.4)
        plt.tight_layout()
        outn = PLOT_DIR / f"{fname_}_correct_init.png"
        plt.savefig(outn)
        plt.close()
...
if __name__ == "__main__":
    ...
    if not args.no_analysis:
        ...
        all_stats = {}
        for model_name, model_obj in MODELS.items():
            ck_path = CHK_DIR / f"{DATASET}_{model_name}_correct_init.pth"
        ...
        # Define color scheme and line styles for v5 (only 14-layer models)
        colors = {
        ...
        line_widths = {
            14: 2.5,      # Slightly thicker since we only have one depth
        }
        ...
        for tag, stats in all_stats.items():
            ...
            line = ax.plot(x_vals, means, 
                          ...
                          linestyle=linestyle, 
                          linewidth=linewidth,
                          label=f'{model_type.upper()}-{depth} (channels: 8-16-32-64)',
                          marker='o',
                          markersize=8,
        ...
        ax.set_title(f'Per-Block Activation Analysis: ResNet-14 vs Plain-14 (Reduced Channels)\n'
                    f'Dataset: {DATASET.upper()} | Channel progression: [8, 16, 32, 64] | Error bands show ±1σ',
        ...
        # Single legend since we only have two models
        legend = ax.legend(legend_elements, [el.get_label() for el in legend_elements],
                          title='Model Architectures (v5)', loc='upper right',
        ...
        output_path = PLOT_DIR / f"combined_l2_professional_{DATASET}_correct_init.png"
