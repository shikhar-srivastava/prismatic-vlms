...
This revision adds parameter statistics, activation histograms (linear and log
count), and clarifies the expected tensor shapes during inference.
"""
...
def analyse_model(model_id: str) -> Dict[str, List[float]]:
    ...
    def _plot_hist(matrix, edges, *, log=False):
        if matrix.ndim != 2:
            return
        plt.figure(figsize=(10, 0.6 * matrix.shape[0] + 2), dpi=140)
        data = np.log10(matrix + 1) if log else matrix
        plt.imshow(
            data,
            aspect="auto",
            origin="lower",
        ...
        c_label = "log10(count+1)" if log else "Count"
        plt.colorbar(label=c_label)
        plt.xlabel("Activation value")
        plt.ylabel("Layer")
        title = f"{model_id} activation histogram"
        if log:
            title += " (log scale)"
        plt.title(title, weight="bold")
        plt.tight_layout()
        tag = "hist_log" if log else "hist"
        p = OUT_DIR / f"{safe}_{tag}.png"
        plt.savefig(p)
        plt.close()
        print("    Plot ", p)
    ...
    _plot_hist(hist_matrix, hist_edges, log=True)
    return stats
...
if all_stats:
    ...
    def _combined_plot(key: str, title: str, ylabel: str, tag: str):
        plt.figure(figsize=(12, 6), dpi=150)
        for mid, s in all_stats.items():
            plt.plot(range(1, len(s[key]) + 1), s[key], label=mid, lw=1.4)
        plt.title(title, weight="bold")
        plt.xlabel("Layer")
        plt.ylabel(ylabel)
        plt.grid(True, ls="--", lw=0.4)
        ...
        plt.tight_layout()
        path = OUT_DIR / f"combined_{tag}.png"
        plt.savefig(path)
        plt.close()
        print("\nCombined plot ", path)

    _combined_plot("l2_mean", "Layerwise activation L2 (mean)", "L2 norm", "l2")
    _combined_plot("raw_mean", "Layerwise raw activation (mean)", "Activation", "raw")
    _combined_plot("param_l2_mean", "Layerwise parameter L2 (mean)", "L2 norm", "param_l2")
    _combined_plot("param_raw_mean", "Layerwise raw parameter (mean)", "Parameter", "param_raw")
else:
