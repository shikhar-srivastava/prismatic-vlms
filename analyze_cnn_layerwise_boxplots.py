...
def _get_blocks(model: torch.nn.Module, tag: str):
    if tag == "resnet":
        return [b for layer in [model.layer1, model.layer2,
                                model.layer3, model.layer4] for b in layer]
    if tag == "densenet":
        feats = model.features
        dbs   = [feats.denseblock1, feats.denseblock2,
                 feats.denseblock3, feats.denseblock4]
        return [b for db in dbs for b in db]           # DenseLayer only
    return [m for m in model.features                 # VGG convs
            if isinstance(m, torch.nn.Conv2d)]

def _param_stats(m: torch.nn.Module) -> Tuple[float,float,float,float]:
    vecs = [p.detach().float().view(-1) for p in m.parameters()
            if p.requires_grad]
    if not vecs:
        return 0,0,0,0
    flat = torch.cat(vecs)
...
def _load_model(model_id: str, device: torch.device) -> torch.nn.Module:
    ...
    enum = next((getattr(tvm, n) for n in dir(tvm)
                 if n.lower() == enum_name), None)
...
def analyse(tag: str,
            model_id: str,
            max_blocks: Optional[int]) -> Dict[str, List[float]]:
    ...
    for b in blocks:                                  # parameter stats
        ...
        par_L2_m.append(m1); par_L2_s.append(s1)
        par_raw_m.append(m2); par_raw_s.append(s2)
    ...
    for a in activations:
        if a.ndim == 3:                               # (C,H,W)
            C,H,W = a.shape
            mu,s  = a.mean(), a.std()
            idxs  = np.argwhere((a > 4 * mu) | (a < -4 * mu))
            outs  = [(float(a[c,r,w]), r*W+w, c) for c,r,w in idxs]
    ...
    stats = {"l2_mean":act_L2_m,"l2_std":act_L2_s,
             "raw_mean":act_raw_m,"raw_std":act_raw_s,
             "param_l2_mean":par_L2_m,"param_l2_std":par_L2_s,
             "param_raw_mean":par_raw_m,"param_raw_std":par_raw_s}
    (OUT_DIR/f"{safe}.json").write_text(json.dumps(stats))

    def _plot(vals, stds, ttl, ylab, tag):
        x = range(1,len(vals)+1)
        plt.figure(figsize=(10,4),dpi=300)
        plt.fill_between(x, [v-s for v,s in zip(vals,stds)],
                           [v+s for v,s in zip(vals,stds)], alpha=.25)
        plt.plot(x, vals, marker='o')
        plt.title(ttl, weight='bold'); plt.xlabel("Block"); plt.ylabel(ylab)
        plt.grid(True, ls='--', lw=.4); plt.tight_layout()
        plt.savefig(OUT_DIR/f"{safe}_{tag}.png"); plt.close()

    def _plot_box(data, outs):
        if not data: return
        plt.figure(figsize=(12, 0.5*len(data)+3), dpi=300)
        plt.boxplot([d.flatten() for d in data], vert=True, patch_artist=True,
                    showfliers=True,
                    flierprops={'marker':'o','markersize':2,
                                'markerfacecolor':'r','alpha':.6})
        for i,o in enumerate(outs):
            if not o: continue
            xs = np.full(len(o), i+1); ys=[v for v,_,_ in o]
            plt.scatter(xs, ys, c='red', marker='x')
            for x,y,pid,ch in zip(xs,ys,[p for _,p,_ in o],[c for _,_,c in o]):
                plt.text(x+.1, y, f"p{pid} [d{ch}]", fontsize=6, ha='left')
        plt.xlabel("Block"); plt.ylabel("Activation value")
        plt.title(f"{label} activation distribution", weight='bold')
        plt.grid(True, ls='--', lw=.4, axis='y'); plt.tight_layout()
        plt.savefig(OUT_DIR/f"{safe}_box.png"); plt.close()
...
for depth in ("top16", "full"):
    ...
    if not all_stats:
        print(f"No successful runs for {depth}"); continue
    ...
    def _combine(metric, ylab, fname, title):
        ...
        plt.title(title, weight='bold'); plt.xlabel("Block"); plt.ylabel(ylab)
        plt.grid(True, ls='--', lw=.4); plt.legend(fontsize='x-small')
        plt.tight_layout(); plt.savefig(OUT_DIR/f"{fname}_{depth}.png"); plt.close()

    _combine("l2_mean",        "L2 norm",        "combined_l2",
             "Blockwise activation L2 (mean)")
    _combine("raw_mean",       "Activation",     "combined_raw",
             "Blockwise raw activation (mean)")
    _combine("param_l2_mean",  "Parameter L2",   "combined_param_l2",
             "Blockwise parameter L2 (mean)")
    _combine("param_raw_mean", "Parameter value","combined_param_raw",
             "Blockwise raw parameter (mean)")
