#!/usr/bin/env python3
# train_and_analyze_resnet_variants_v3.py
#
# Purpose:
...
# ───────────────────────── CONFIG ─────────────────────────
BASE = Path.cwd()
CHK_DIR = BASE / "checkpoints_v3"
CHK_DIR.mkdir(exist_ok=True)

CURVE_DIR = BASE / "viz_v3"
CURVE_DIR.mkdir(exist_ok=True)

PLOT_DIR = BASE / "viz_v3" / "plots" / "act_analysis_rvp_v3"
...
def plot_training_curves(tag: str, train_losses: List[float], train_accs: List[float], 
    ...
    plt.tight_layout()
    curve_path = CURVE_DIR / f"{DATASET}_{tag}_training_curves_v3.png"
...
def train(tag: str, model: nn.Module, train_loader: DataLoader, val_loader: DataLoader):
    ck = CHK_DIR / f"{DATASET}_{tag}_v3.pth"
    if ck.exists():
...
def _plot_box(activations: List[np.ndarray],
    ...
    plt.tight_layout()
    outpath = PLOT_DIR / f"{label}_box_v3.png"
    plt.savefig(outpath)
    plt.close()
...
def analyse(tag: str, model: nn.Module, results: Dict[str, Dict[str, List[float]]]):
    ...
    results[tag] = stats
    (PLOT_DIR / f"{DATASET}_{tag}_v3.json").write_text(json.dumps(stats))
    ...
    def _plot_line(vals, stds, title_, ylabel_, fname_):
        ...
        plt.grid(ls="--", lw=0.4)
        plt.tight_layout()
        outn = PLOT_DIR / f"{fname_}_v3.png"
        plt.savefig(outn)
        plt.close()
...
if __name__ == "__main__":
    ...
    if not args.no_analysis:
        ...
        all_stats = {}
        for model_name, model_obj in MODELS.items():
            ck_path = CHK_DIR / f"{DATASET}_{model_name}_v3.pth"
        ...
        # Define color scheme and line styles for v3 (only 14-layer models)
        colors = {
        ...
        legend = ax.legend(legend_elements, [el.get_label() for el in legend_elements],
                          title='Model Architectures (v3)', loc='upper right',
        ...
        output_path = PLOT_DIR / f"combined_l2_professional_{DATASET}_v3.png"
